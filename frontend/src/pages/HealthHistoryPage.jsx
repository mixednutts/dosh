import { useEffect, useMemo, useState } from 'react'
import { Link, Navigate, useSearchParams } from 'react-router-dom'
import { useQuery } from '@tanstack/react-query'
import clsx from 'clsx'
import { getBudgets, getPeriodsForBudget, getHealthHistoryTrends } from '../api/client'
import HealthHistoryChart from '../components/reports/HealthHistoryChart'
import CycleFilter from '../components/reports/CycleFilter'
import Spinner from '../components/Spinner'

function formatUTCDate(date) {
  const y = date.getUTCFullYear()
  const m = String(date.getUTCMonth() + 1).padStart(2, '0')
  const d = String(date.getUTCDate()).padStart(2, '0')
  return `${y}-${m}-${d}`
}

function MetricToggle({ metric, checked, onChange, color }) {
  return (
    <button
      type="button"
      onClick={() => onChange(metric.key, !checked)}
      className={clsx(
        'flex w-full items-center gap-2 rounded-xl border px-3 py-2 text-left text-xs font-medium transition-colors',
        checked
          ? 'border-dosh-400 bg-dosh-600 text-white'
          : 'border-gray-200 bg-white text-gray-600 hover:border-dosh-300 hover:bg-dosh-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-400 dark:hover:border-dosh-700 dark:hover:bg-slate-800'
      )}
    >
      <span
        className={clsx('inline-block h-2.5 w-2.5 shrink-0 rounded-full', checked ? 'bg-white' : 'bg-gray-300 dark:bg-slate-600')}
        style={checked ? undefined : { backgroundColor: color }}
      />
      <span className="min-w-0 truncate">{metric.name}</span>
    </button>
  )
}

export default function HealthHistoryPage() {
  const [searchParams] = useSearchParams()
  const budgetId = Number.parseInt(searchParams.get('budgetId') || '', 10) || null

  const { data: budgets = [], isLoading: budgetsLoading } = useQuery({
    queryKey: ['budgets'],
    queryFn: getBudgets,
    staleTime: 60_000,
  })

  const { data: periods = [] } = useQuery({
    queryKey: ['periods', budgetId],
    queryFn: () => getPeriodsForBudget(budgetId),
    enabled: !!budgetId,
    staleTime: 60_000,
  })

  const [filterParams, setFilterParams] = useState({ fromDate: null, toDate: null })

  const nonPlannedPeriods = useMemo(() =>
    periods.filter(p => p.cycle_stage !== 'PLANNED'),
  [periods])

  const queryParams = useMemo(() => {
    const params = {}
    if (filterParams.fromDate) {
      params.from_date = formatUTCDate(filterParams.fromDate)
    }
    if (filterParams.toDate) {
      params.to_date = formatUTCDate(filterParams.toDate)
    }
    return params
  }, [filterParams])

  const { data: historyData, isLoading: historyLoading } = useQuery({
    queryKey: ['health-history-trends', budgetId, queryParams],
    queryFn: () => getHealthHistoryTrends(budgetId, queryParams),
    enabled: !!budgetId,
    staleTime: 60_000,
  })

  const currentBudget = budgets.find(b => b.budgetid === budgetId) || null

  const metrics = historyData?.metrics || []
  const rawPeriods = historyData?.periods || []

  const [visibleMetrics, setVisibleMetrics] = useState(() => new Set())

  // Default all metrics to visible when data first loads
  useEffect(() => {
    if (metrics.length > 0 && visibleMetrics.size === 0) {
      setVisibleMetrics(new Set(metrics.map(m => m.key)))
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [metrics])

  const toggleMetric = (key, checked) => {
    setVisibleMetrics(prev => {
      const next = new Set(prev)
      if (checked) {
        next.add(key)
      } else {
        next.delete(key)
      }
      return next
    })
  }

  const chartData = useMemo(() => {
    return rawPeriods.map(p => ({
      label: p.label,
      ...p.scores,
    }))
  }, [rawPeriods])

  if (!budgetId) {
    if (budgetsLoading) {
      return (
        <div className="flex min-h-[40vh] items-center justify-center">
          <Spinner className="h-8 w-8" />
        </div>
      )
    }
    if (budgets.length === 0) {
      return (
        <div className="space-y-6">
          <h1 className="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Health History</h1>
          <p className="text-sm text-gray-500 dark:text-slate-400">No budgets available.</p>
        </div>
      )
    }
    return <Navigate to={`/reports/health-history?budgetId=${budgets[0].budgetid}`} replace />
  }

  const metricColorMap = {}
  const themeColors = ['#0ea5e9', '#0284c7', '#f97316', '#ea580c', '#8b5cf6', '#7c3aed', '#10b981', '#059669']
  metrics.forEach((m, i) => {
    metricColorMap[m.key] = themeColors[i % themeColors.length]
  })

  return (
    <div className="space-y-6">
      <nav className="flex flex-wrap items-center gap-2 text-sm">
        <Link to="/budgets" className="text-dosh-600 hover:underline dark:text-dosh-400">Budgets</Link>
        <span className="text-gray-400 dark:text-slate-600">/</span>
        <Link to={`/budgets/${budgetId}`} className="text-dosh-600 hover:underline dark:text-dosh-400">
          {currentBudget?.description || 'Budget'}
        </Link>
        <span className="text-gray-400 dark:text-slate-600">/</span>
        <Link to={`/reports/${budgetId}`} className="text-dosh-600 hover:underline dark:text-dosh-400">Reports</Link>
        <span className="text-gray-400 dark:text-slate-600">/</span>
        <span className="font-medium text-gray-900 dark:text-white">Health History</span>
      </nav>

      <div className="space-y-1">
        <h1 className="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Reports</h1>
        <p className="text-sm text-gray-500 dark:text-slate-400">Health History</p>
      </div>

      <div className="rounded-2xl border border-gray-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-950/85 lg:p-6">
        <div className="mb-4 space-y-1.5">
          <span className="text-[10px] font-semibold uppercase tracking-wider text-gray-400 dark:text-slate-500">Range</span>
          <CycleFilter
            budgetPeriods={nonPlannedPeriods}
            onChange={setFilterParams}
            defaultPreset="last12"
          />
        </div>

        {historyLoading ? (
          <div className="flex min-h-[20rem] items-center justify-center">
            <Spinner className="h-8 w-8" />
          </div>
        ) : chartData.length === 0 ? (
          <div className="flex min-h-[20rem] flex-col items-center justify-center gap-2 text-center">
            <p className="text-sm font-medium text-gray-700 dark:text-slate-300">No health history to display</p>
            <p className="text-xs text-gray-500 dark:text-slate-400">Try adjusting the filter to include more budget cycles.</p>
          </div>
        ) : (
          <div className="flex flex-col gap-4 lg:flex-row">
            <div className="flex-1 min-w-0">
              <HealthHistoryChart
                data={chartData}
                metrics={metrics}
                visibleMetrics={visibleMetrics}
              />
            </div>
            {metrics.length > 0 && (
              <div className="shrink-0 space-y-2 lg:w-48">
                <span className="block text-[10px] font-semibold uppercase tracking-wider text-gray-400 dark:text-slate-500">Metrics</span>
                <div className="space-y-1.5">
                  {metrics.map(metric => (
                    <MetricToggle
                      key={metric.key}
                      metric={metric}
                      checked={visibleMetrics.has(metric.key)}
                      onChange={toggleMetric}
                      color={metricColorMap[metric.key]}
                    />
                  ))}
                </div>
              </div>
            )}
          </div>
        )}
      </div>
    </div>
  )
}
