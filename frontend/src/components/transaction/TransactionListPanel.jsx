        ...
        return (
          <div key={item.id} className="flex items-center gap-2 px-3 py-2">
            ...
            <div className="min-w-0 flex-1">
              ...
              {(item.related_account_desc || item.affected_account_desc) && (
                <p className="truncate text-xs text-gray-500 dark:text-gray-400">
                  {item.related_account_desc ? `From ${item.related_account_desc}` : ''}
                  {item.related_account_desc && item.affected_account_desc ? ' → ' : ''}
                  {item.affected_account_desc ? `To ${item.affected_account_desc}` : ''}
                </p>
              )}
