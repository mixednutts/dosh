...
describe('CycleFilter', () => {
  ...
  it('renders preset buttons', () => {
    renderFilter()
    expect(screen.getByRole('button', { name: 'Last 12 Cycles' })).toBeTruthy()
    expect(screen.getByRole('button', { name: 'Last 6 Cycles' })).toBeTruthy()
    expect(screen.getByRole('button', { name: 'Last 3 Cycles' })).toBeTruthy()
    expect(screen.getByRole('button', { name: 'All Cycles' })).toBeTruthy()
  })
  ...
  it('switches preset when a different button is clicked', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Last 6 Cycles' }))
  ...
  it('applies last3 preset correctly', async () => {
    renderFilter({ defaultPreset: 'all' })
    await waitFor(() => expect(mockOnChange).toHaveBeenCalled())
    mockOnChange.mockClear()

    fireEvent.click(screen.getByRole('button', { name: 'Last 3 Cycles' }))
