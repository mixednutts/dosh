...
def test_get_health_matrix(client, db_session):
    ...
    assert metric_keys == {"setup_health", "budget_discipline"}
...
def test_update_matrix_item(client, db_session):
    ...
    assert response.status_code == 200
    data = response.json()
    metric_id = data["items"][0]["metric_id"]
    ...
    item = next(i for i in updated["items"] if i["metric_id"] == metric_id)
...
def test_update_matrix_item_parameters(client, db_session):
    ...
    metric_id = setup_item["metric_id"]
...
def test_update_matrix_item_not_found(client, db_session):
    budget = create_budget(db_session)
    response = client.patch(
        f"/api/budgets/{budget.budgetid}/health-matrix/items/99999",
        json={"weight": 0.5},
    )
