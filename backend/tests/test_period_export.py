...
def test_period_export_returns_404_for_missing_period(client):
    response = client.get("/api/periods/999999/export?format=csv")
...
def test_period_export_csv_includes_transaction_budget_adjustment_and_budget_only_rows(client, db_session):
    ...
    income_tx = client.post(
        f"/api/periods/{period_id}/income/Salary/transactions/",
    ...
    budget_adjustment = client.patch(
        f"/api/periods/{period_id}/income/Salary/budget",
    ...
    detail_response = client.get(f"/api/periods/{period_id}")
    ...
    response = client.get(f"/api/periods/{period_id}/export?format=csv")
...
def test_period_export_json_preserves_closed_cycle_history(client, db_session):
    ...
    response = client.get(f"/api/periods/{active_period['finperiodid']}/export?format=json")
    assert response.status_code == 200, response.text
