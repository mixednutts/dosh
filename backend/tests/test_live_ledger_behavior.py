...
def test_direct_actual_updates_create_system_ledger_rows_and_source_filters(client, db_session):
    ...
    income_only = client.get(f"/api/periods/{finperiodid}/transactions?source=income")
    ...
    expense_only = client.get(f"/api/periods/{finperiodid}/transactions?source=expense&source_key=Rent")
